classdef Camera < handle
    ...
    methods
      ...
      methods (Static)
        function dist = get_distance_to_centroid ( centroid, endEffector)
            dist = power((power(centroid(1) - endEffector(1), 2) + ...
                power(centroid(2) - endEffector(2), 2) + ...
                power(centroid(3) - endEffector(3), 2) ), 0.5);

        end


        function closest = get_closest_ball (listOfCentroids, endEffectorPosition)
                
                minDist = 10000;
                minCentroid = struct;

                len = size(listOfCentroids);

                for i = 1:len(1)
                    c = listOfCentroids(i,:);
%                     disp(c)
                    dist = Camera.get_distance_to_centroid(c, endEffectorPosition);
                    if (dist < minDist)
                        minCentroid = c;
                        minDist = dist;
